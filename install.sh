...
echo ""
echo "======================================================"
echo "🧪 COMO TESTAR AS NOTIFICAÇÕES AGORA MESMO:"
echo "======================================================"
...
if [ "$IS_PANEL" = true ]; then
    echo "▶ PARA O PAINEL:"
    echo "1. Para testar o Webhook simulando que há uma versão nova:"
    echo "   echo \"v0.0.0\" > /tmp/ptero_current_version && /usr/local/bin/pteroupdate.sh check"
    echo "   (Isso fará o sistema achar que está na versão 0.0.0 e enviará o alerta no Discord)"
    echo ""
    echo "2. Para rodar o comando de atualização real:"
    echo "   update_painel"
    echo ""
fi
...
if [ "$IS_WINGS" = true ]; then
    echo "▶ PARA O WINGS:"
    echo "1. Para testar o Webhook simulando que há uma versão nova:"
    echo "   echo \"v0.0.0\" > /tmp/wings_current_version && /usr/local/bin/wingsupdate.sh check"
    echo "   (Isso fará o sistema achar que está na versão 0.0.0 e enviará o alerta no Discord)"
    echo ""
    echo "2. Para rodar o comando de atualização real:"
    echo "   update_wings"
